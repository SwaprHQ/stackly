...
export function Mint() {
  ...
  let mintedAmount : BigNumber = BigNumber.from(0);
  ...
  let chainId : number = chain ? chain.id : 0;
  let nftHolder : boolean = false;

  const fetchMinted = async () => {
    if (signer) {
      const nftWhitelist = getWhitelist(getNftWhitelistAddress(chainId), signer);
      mintedAmount = await nftWhitelistTotalSupply(nftWhitelist);
  ...
  const isNftHolder = async () => {
    if (signer && account.address) {
      const nftWhitelist = getWhitelist(getNftWhitelistAddress(chainId), signer);
      let amount = await nftWhitelistBalanceOf(nftWhitelist, account.address);
      nftHolder = amount.gt(0);
    }
  }
  ...
  const mint = async () => {
    if (signer && account.address) {
      const nftWhitelist = getWhitelist(getNftWhitelistAddress(chainId), signer);
      let tx = await nftWhitelistMint(nftWhitelist);
      console.log(tx);
    }
  }
    ...
    <Container>
      ...
      <ButtonContainer>
        <Button
          ...
          title="Mint beta NFT"
          onClick={mint}
          style={{display:(nftHolder ? 'none' : '')}}>
          {<>Mint</>}
        </Button>
      </ButtonContainer>
      
      <Text>{nftHolder ? "Congratulations, already hold the Stackly Beta NFT" : ""}</Text>
      <Text>{mintedAmount.gt(0) ? mintedAmount.toString() + " NFTs have been minted so far" : ""}</Text>
