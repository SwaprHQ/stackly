...
import { getERC20Byte32Contract } from 'dca-sdk';
import {
  ChainId,
  Currency,
  USDC,
...
export function useTokenFromActiveNetwork(
  ...
  const tokenContract = useMemo(() => {
    return formattedAddress
      ? getERC20Contract(formattedAddress, provider)
      : null;
  }, [provider, formattedAddress]);

  const tokenContractBytes32 = useMemo(() => {
    return formattedAddress
      ? getERC20Byte32Contract(formattedAddress, provider)
      : null;
  }, [provider, formattedAddress]);
...
export function useCurrencyFromMap(
  ...
  const shorthandMatchAddress = useMemo(() => {
    return chainId && currencyId
      ? TOKEN_SHORTHANDS[currencyId.toUpperCase()]?.[chainId]
      : undefined;
  }, [chainId, currencyId]);
  ...
  if (currencyId === null || currencyId === undefined || chain?.unsupported)
    return null;
