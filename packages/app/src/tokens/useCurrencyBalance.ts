import { Currency, Amount as CurrencyAmount, Token, isAddress, getERC20Interface } from 'dca-sdk';
// import {
...
import { nativeOnChain } from './utils';
import { useMulticallContract } from '../lib/state/multicall';
...
export function useTokenBalancesWithLoadingIndicator(
  ...
  const chainId = useNetwork().chain?.id as number; // we cannot fetch balances cross-chain
...
export function useCurrencyBalanceString(account: string): string {
  return (
    useNativeCurrencyBalances(account ? [account] : [])?.[
      account ?? ''
    ]?.toFixed(3) ?? ''
  );
}
