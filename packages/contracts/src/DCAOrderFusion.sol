...
import {IFusionSettlement} from "./interfaces/IFusionSettlement.sol";
import {IDCAOrder} from "./interfaces/IDCAOrder.sol";
import {FusionOrder} from "./libraries/FusionOrder.sol";
...
contract DCAOrder is EIP1271Verifier, IDCAOrder {
  using FusionOrder for FusionOrder.Data;
  ...
  function initialize(
    ...
    interval = _interval;
    amount = _amount;
    domainSeparator = IFusionSettlement(_settlementContract).domainSeparator();
    
    // Approve the vaut relayer to spend the sell token
    IERC20(_sellToken).safeApprove(address(IFusionSettlement(_settlementContract).vaultRelayer()), type(uint256).max);
  ...
  function getTradeableOrder() external view returns (FusionOrder.Data memory) {
    ...
    return FusionOrder.Data(
      ...
      0,
      FusionOrder.KIND_SELL,
      false,
      FusionOrder.BALANCE_ERC20,
      FusionOrder.BALANCE_ERC20
    );
  ...
  function isValidSignature(bytes32 orderDigest, bytes calldata encodedOrder) external view override returns (bytes4) {
    FusionOrder.Data memory order = abi.decode(encodedOrder, (FusionOrder.Data));
    ...
    require(
      this.getTradeableOrder().hash(domainSeparator) == orderDigest,
